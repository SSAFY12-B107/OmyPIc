...
/**
* Background-Survey 함수 - 백그라운드 설문조사 업데이트
* @param surveyData 백그라운드 설문조사 데이터
*/
export const updateBackgroundSurvey = async <T>(surveyData: any): Promise<AxiosResponse<ApiResponse<T>>> => {
    return apiService.patch<T>('api/auth/naver/background-survey', surveyData)
}
